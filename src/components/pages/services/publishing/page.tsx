...
const ctaStyles = `
  ...
  @keyframes floatMockup { 0%, 100% { transform: translateY(0px); } 50% { transform: translateY(-14px); } }
  ...
  .cta-btn-primary { font-family: 'Montserrat', sans-serif; letter-spacing: 0.12em; font-size: 1rem; padding: 14px 38px; border-radius: 999px; background: white; color: #FF4545; border: none; cursor: pointer; transition: transform 0.25s ease, box-shadow 0.25s ease, background 0.25s ease; box-shadow: 0 6px 28px rgba(255,255,255,0.15); white-space: nowrap; }
  .cta-btn-primary:hover { transform: translateY(-3px); box-shadow: 0 12px 40px rgba(255,255,255,0.25); background: #fff0f0; }
  .cta-btn-secondary { font-family: 'Montserrat', sans-serif; letter-spacing: 0.08em; font-size: 1rem; padding: 12px 38px; border-radius: 999px; background: transparent; color: rgba(255,255,255,0.75); border: 1px solid rgba(255,255,255,0.25); cursor: pointer; transition: transform 0.25s ease, border-color 0.25s ease, color 0.25s ease; white-space: nowrap; }
  .cta-btn-secondary:hover { transform: translateY(-3px); border-color: rgba(255,255,255,0.6); color: white; }
`;
...
const contactStyles = `
  ...
  @keyframes fadeUpContact { from { opacity: 0; transform: translateY(30px); } to { opacity: 1; transform: translateY(0); } }
  @keyframes fadeLeft { from { opacity: 0; transform: translateX(-40px); } to { opacity: 1; transform: translateX(0); } }
  @keyframes fadeRight { from { opacity: 0; transform: translateX(40px); } to { opacity: 1; transform: translateX(0); } }
  @keyframes rotateSlow { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
  @keyframes orbPulse { 0%, 100% { opacity: 0.45; transform: scale(1); } 50% { opacity: 0.75; transform: scale(1.1); } }
  ...
  .field-input { width: 100%; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.1); border-radius: 14px; padding: 18px 20px 18px 56px; color: white; font-family: 'DM Sans', sans-serif; font-size: 0.95rem; font-weight: 300; outline: none; transition: border-color 0.3s ease, background 0.3s ease, box-shadow 0.3s ease; box-sizing: border-box; caret-color: #FF4545; }
  .field-input::placeholder { color: rgba(255,255,255,0.25); }
  .field-input:focus { border-color: rgba(255,69,69,0.6); background: rgba(255,69,69,0.04); box-shadow: 0 0 0 3px rgba(255,69,69,0.1), 0 8px 32px rgba(0,0,0,0.3); }
  .field-input:focus + .field-icon { color: #FF4545 !important; }
  .field-icon { position: absolute; left: 18px; top: 50%; transform: translateY(-50%); color: rgba(255,255,255,0.25); transition: color 0.3s ease; pointer-events: none; display: flex; align-items: center; }
  .field-icon-textarea { top: 20px; transform: none; }
  .field-label { position: absolute; left: 56px; top: 18px; font-family: 'DM Sans', sans-serif; font-size: 0.92rem; color: rgba(255,255,255,0.25); pointer-events: none; transition: all 0.25s cubic-bezier(0.22,1,0.36,1); transform-origin: left; }
  .field-input:focus ~ .field-label, .field-input:not(:placeholder-shown) ~ .field-label { top: -10px; left: 14px; font-size: 0.72rem; color: #FF4545; background: #0a0e2a; padding: 0 6px; border-radius: 4px; letter-spacing: 0.06em; }
  textarea.field-input { resize: none; padding-top: 18px; line-height: 1.65; min-height: 140px; }
  .submit-btn { width: 100%; padding: 18px; border-radius: 14px; background: linear-gradient(90deg, #FF4545 0%, #fe5858 100%); color: white; font-family: 'Bebas Neue', sans-serif; font-size: 1.1rem; letter-spacing: 0.15em; border: none; cursor: pointer; position: relative; overflow: hidden; transition: transform 0.25s ease, box-shadow 0.25s ease; box-shadow: 0 8px 32px rgba(255,69,69,0.3); display: flex; align-items: center; justify-content: center; gap: 10px; }
  .submit-btn::before { content: ''; position: absolute; inset: 0; background: linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.12) 50%, transparent 100%); transform: translateX(-100%); transition: transform 0.5s ease; }
  ...
  .info-card:hover { transform: translateX(6px); border-color: rgba(255,69,69,0.4) !important; }
`;
...
const INFO_ITEMS = [
    { icon: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.8a19.79 19.79 0 01-3.07-8.7A2 2 0 012 .99h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 8.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" /></svg>), label: "Phone", value: "+99 123 456 789" },
    { icon: (<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><polyline points="22,6 12,13 2,6" /></svg>), label: "Email", value: "info@bristolpublishers.com" },
];

const BookCard: React.FC<BookCardProps> = ({ item, height = 260, width = 180 }) => (
    <div className="portfolio-img-card" style={{ width: `${width}px`, height: `${height}px`, margin: "0 10px" }}>
        <img src={item.src} alt={item.title} style={{ width: "100%", height: "100%", objectFit: "cover" }}
            onError={e => {
                const t = e.currentTarget; t.style.display = "none"; const parent = t.parentElement!;
                parent.style.background = `hsl(${Math.random() * 360}, 25%, 18%)`;
                const fb = document.createElement("div"); fb.style.cssText = `width:100%;height:100%;display:flex;align-items:center;justify-content:center;padding:16px;`;
...
const PUBLISHING_FAQS = [
    { q: "What publishing formats do you offer?", a: "We offer comprehensive publishing in Print (Paperback and Hardcover), eBook (ePub, Mobi, PDF), and Audiobook formats. Our team ensures your book meets all platform specifications globally." },
    { q: "Do I retain the rights to my book?", a: "Absolutely. You retain 100% of your copyrights and royalties. We act as your publishing partner, providing the services you need while you maintain complete ownership and creative control." },
    { q: "How long does the publishing process take?", a: "The timeline varies based on the services required. A standard publishing package typically takes 4 to 8 weeks from manuscript finalization to market availability, including formatting, cover design, and distribution setup." },
    { q: "Will my book be available in bookstores?", a: "Yes! We distribute your book to major retailers including Amazon, Barnes & Noble, Ingram, and Apple Books. This makes your book available to order in physical bookstores worldwide." },
    { q: "Do you provide ISBN and copyright registration?", a: "Yes, we provide a free ISBN for both your print and eBook versions, and we guide you through the copyright registration process to ensure your work is legally protected." },
];
                    ...
                    <div key={i} style={{
                        ...
                        <div style={{
                            ...
                            <img src={b.src} alt="" style={{ width: "100%", height: `${b.h}px`, objectFit: "cover", display: "block", borderRadius: "10px", filter: "brightness(.85) saturate(1.1) contrast(1.05)", boxShadow: "0 24px 60px rgba(0,0,0,.7), inset 0 0 30px rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.1)" }}
                                onError={e => { e.currentTarget.style.display = "none"; const p = e.currentTarget.parentElement as HTMLElement; const absW = p.parentElement as HTMLElement; absW.style.height = `${b.h}px`; absW.style.borderRadius = "10px"; absW.style.background = `linear-gradient(135deg, hsl(${i * 25 + 340},30%,15%), hsl(${i * 25 + 350},40%,22%))`; absW.style.border = "1px solid rgba(255,255,255,.1)"; }} />
                        </div>
                    </div>
                );
            })}
            ...
            <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", position: "relative", zIndex: 4, padding: "120px 360px 100px", paddingTop: "140px", textAlign: "center" }}>
                <div style={{ maxWidth: "820px", width: "100%" }}>
                    <div style={{ display: "inline-flex", alignItems: "center", gap: "12px", marginBottom: "24px", opacity: visible ? 1 : 0, animation: visible ? "ap-fadeUp .5s ease .1s forwards" : "none" }}>
                        ...
                        <span style={{ fontFamily: "'Montserrat',sans-serif", fontSize: ".75rem", letterSpacing: ".3em", color: "#FF4545", fontWeight: 600 }}>PUBLISHING SERVICES</span>
                        <div style={{ width: visible ? "32px" : "0", height: "2px", background: "#FF4545", transition: "width .8s ease .3s" }} />
                    </div>
                    <h1 style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 800, fontSize: "clamp(2rem,3.8vw,3.4rem)", letterSpacing: "-.025em", lineHeight: 1.1, color: "white", margin: "0 0 28px", opacity: visible ? 1 : 0, animation: visible ? "ap-fadeUp .7s ease .2s forwards" : "none" }}>
                        Transform Your Manuscript Into<br /><span style={{ color: "#FF4545" }}>A Published Book</span>
                    </h1>
                    <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: "1.05rem", lineHeight: 1.85, color: "rgba(255,255,255,.52)", fontWeight: 300, margin: "0 auto 38px", maxWidth: "560px", opacity: visible ? 1 : 0, animation: visible ? "ap-fadeUp .7s ease .38s forwards" : "none" }}>
                        Navigate the publishing world with confidence. From formatting and editing to global distribution, we provide end-to-end book publishing services designed to bring your stories to readers everywhere.
                    </p>
                    <div style={{ display: "flex", gap: "14px", justifyContent: "center", flexWrap: "wrap", opacity: visible ? 1 : 0, animation: visible ? "ap-fadeUp .7s ease .5s forwards" : "none" }}>
                        <button className="ap-btn-primary" style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 700, fontSize: ".92rem", letterSpacing: ".08em", padding: "14px 42px", borderRadius: "999px", background: "linear-gradient(90deg,#fe5858 0%,#FF4545 100%)", color: "#fff", border: "none", cursor: "pointer", boxShadow: "0 8px 28px rgba(255,69,69,.4)" }}>Start Publishing</button>
                        <button className="ap-btn-outline" style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 600, fontSize: ".92rem", letterSpacing: ".06em", padding: "13px 42px", borderRadius: "999px", background: "transparent", color: "rgba(255,255,255,.7)", border: "1px solid rgba(255,255,255,.22)", cursor: "pointer" }}>View Packages</button>
                    </div>
                </div>
            </div>
        </section>
...
const PublishingMarquee: React.FC = () => {
    const { ref, visible } = useApInView(0.05);
    return (
        <section ref={ref} style={{ background: "linear-gradient(180deg, #FFFFFF 0%, #FFF9F9 25%, #FFE8E8 55%, #FFD6D6 80%, #FFFFFF 100%)", width: "100%", overflow: "hidden", padding: "100px 0 110px", position: "relative" }}>
            <div style={{ position: "absolute", top: "20%", left: "-6%", width: "450px", height: "450px", borderRadius: "50%", background: "radial-gradient(circle, rgba(255,69,69,0.7) 0%, transparent 65%)", animation: "orbPulse 7s ease-in-out infinite", pointerEvents: "none" }} />
            <div style={{ position: "absolute", bottom: "15%", right: "-5%", width: "380px", height: "380px", borderRadius: "50%", background: "radial-gradient(circle, rgba(255,69,69,0.7) 0%, transparent 70%)", pointerEvents: "none" }} />

            <div style={{ maxWidth: "1200px", margin: "0 auto 60px", padding: "0 40px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "22px", opacity: visible ? 1 : 0, animation: visible ? "fadeUp 0.6s ease forwards" : "none" }}>
                    <div style={{ height: "2px", background: "#FF4545", width: visible ? "48px" : "0", transition: "width 0.8s ease 0.2s" }} />
                    <span style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "0.85rem", letterSpacing: "0.25em", color: "#FF4545" }}>PUBLISHED BOOKS</span>
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: "20px" }}>
                    <h2 style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 800, fontSize: "clamp(2.5rem, 4.5vw, 4.5rem)", letterSpacing: "-0.02em", lineHeight: 0.9, color: "white", margin: 0 }}>
                        {visible && (<>
                            <SplitText text="Recently" className="text-[#0A0A0A]" delay={35} duration={1.1} ease="power3.out" splitType="chars" from={{ opacity: 0, y: 45 }} to={{ opacity: 1, y: 0 }} threshold={0.1} rootMargin="-50px" textAlign="left" />{" "}
                            <SplitText text="Published" className="text-[#FF4545]" delay={42} duration={1.2} ease="power3.out" splitType="chars" from={{ opacity: 0, y: 45 }} to={{ opacity: 1, y: 0 }} threshold={0.1} rootMargin="-50px" textAlign="left" />
                        </>)}
                    </h2>
                    <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "1.1rem", lineHeight: 1.75, color: "#0A0A0A", maxWidth: "340px", margin: 0, fontWeight: 300, opacity: visible ? 1 : 0, animation: visible ? "fadeUp 0.7s ease 0.3s forwards" : "none" }}>
                        We have helped hundreds of authors bring their manuscripts to life. Browse through our collection of professionally published works spanning multiple genres and formats.
                    </p>
                </div>
            </div>
            ...
            <div style={{ position: "relative", marginBottom: "16px" }}>
                <MarqueeRow direction="left" speed={1.2}>
                    {ROW_TOP.map((item, i) => (<BookCard key={i} item={item} height={400} width={250} />))}
                </MarqueeRow>
            </div>

            <div style={{ position: "relative" }}>
                <MarqueeRow direction="right" speed={1.2}>
                    {ROW_BOTTOM.map((item, i) => (<BookCard key={i} item={item} height={400} width={250} />))}
                </MarqueeRow>
            </div>
        </section>
    );
    ...
    return (
        <>
            ...
            <section style={{ background: "linear-gradient(180deg, #1B465F 0%, #14384C 100%)", width: "100%", padding: "80px 40px 90px", position: "relative", overflow: "hidden" }}>
                <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", width: "700px", height: "300px", borderRadius: "50%", background: "radial-gradient(ellipse, rgba(255,69,69,0.07) 0%, transparent 70%)", pointerEvents: "none" }} />
                <div ref={ref} style={{ maxWidth: "1100px", margin: "0 auto", borderRadius: "28px", position: "relative", overflow: "hidden", padding: "2px", background: "linear-gradient(135deg, rgba(255,69,69,0.5), rgba(26,36,95,0.8), rgba(255,69,69,0.3))", backgroundSize: "300% 300%", animation: "shimmerBorder 5s ease infinite" }}>
                    <div style={{ borderRadius: "26px", background: "linear-gradient(125deg, #1B465F 0%, #16394D 40%, #102A3A 100%)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "52px 60px 52px 64px", overflow: "hidden", position: "relative", minHeight: "260px" }}>
                        <div style={{ position: "absolute", top: "-40%", left: "-5%", width: "350px", height: "350px", borderRadius: "50%", background: "radial-gradient(circle, rgba(255,69,69,0.09) 0%, transparent 65%)", pointerEvents: "none" }} />
                        <div style={{ position: "absolute", bottom: "-30%", right: "25%", width: "300px", height: "300px", borderRadius: "50%", background: "radial-gradient(circle, rgba(26,36,95,0.6) 0%, transparent 70%)", pointerEvents: "none" }} />
                        ...
                        <div style={{ position: "relative", zIndex: 5, maxWidth: "540px", flex: "1" }}>
                            <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", marginBottom: "22px", opacity: visible ? 1 : 0, animation: visible ? "fadeUp 0.5s ease forwards" : "none" }}>
                                <span style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#FF4545", display: "inline-block", animation: "pulseDot 2s ease-in-out infinite" }} />
                                <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "0.78rem", letterSpacing: "0.2em", color: "rgba(255,255,255,0.5)" }}>PUBLISH YOUR BOOK</span>
                            </div>
                            <h2 style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 800, fontSize: "clamp(2rem, 4.5vw, 3rem)", letterSpacing: "-0.01em", lineHeight: 1.03, color: "white", margin: "0 0 18px" }}>
                                {visible && (<>
                                    <SplitText text="Your Manuscript Is Not" delay={30} duration={1.0} ease="power3.out" splitType="chars" from={{ opacity: 0, y: 35 }} to={{ opacity: 1, y: 0 }} threshold={0.1} rootMargin="-30px" textAlign="left" /><br />
                                    <SplitText text="Meant To Stay" delay={35} duration={1.05} ease="power3.out" splitType="chars" from={{ opacity: 0, y: 35 }} to={{ opacity: 1, y: 0 }} threshold={0.1} rootMargin="-30px" textAlign="left" /><br />
                                    <SplitText text="Unpublished" className="text-[#FF4545]" delay={40} duration={1.1} ease="power3.out" splitType="chars" from={{ opacity: 0, y: 35 }} to={{ opacity: 1, y: 0 }} threshold={0.1} rootMargin="-30px" textAlign="left" />
                                </>)}
                            </h2>
                            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "1.1rem", lineHeight: 1.75, color: "rgba(255,255,255,0.5)", margin: "0 0 34px", fontWeight: 300, maxWidth: "420px", opacity: visible ? 1 : 0, animation: visible ? "fadeUp 0.7s ease 0.35s forwards" : "none" }}>
                                Take the next step with confidence. Let us help you publish your manuscript, distribute it globally, and reach the readers who are waiting for your story.
                            </p>
                            <div style={{ display: "flex", gap: "14px", alignItems: "center", flexWrap: "wrap", opacity: visible ? 1 : 0, animation: visible ? "fadeUp 0.7s ease 0.45s forwards" : "none" }}>
                                <button className="px-10 py-3.5 rounded-full text-white text-sm transition-all duration-200 hover:opacity-90" style={{ fontFamily: "'Montserrat', sans-serif", letterSpacing: "0.25em", background: "linear-gradient(90deg, #fe5858e8 0%, #FF4545 100%)" }}>Get Started</button>
                                <button className="cta-btn-secondary">LEARN MORE</button>
                            </div>
                        </div>

                        <div style={{ position: "absolute", right: "40px", bottom: "20px", height: "400px", zIndex: 4, animation: "floatMockup 5s ease-in-out infinite", filter: `drop-shadow(-20px 10px 50px rgba(0,0,0,0.7)) drop-shadow(0 0 30px rgba(255,69,69,0.12))`, pointerEvents: "none" }}>
                            <img src="/images/Portfolio/MOCKUP.png" alt="Published Book Mockup" style={{ height: "400px", width: "auto", objectFit: "contain", objectPosition: "top right" }} />
                        </div>
                    </div>
                </div>
            </section>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 400px", gap: "80px", alignItems: "center" }}>
                    <div style={{ opacity: visible ? 1 : 0, animation: visible ? "ap-fadeLeft .85s ease .2s forwards" : "none" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "22px" }}>
                            ...
                            <span style={{ fontFamily: "'Montserrat',sans-serif", fontSize: ".8rem", letterSpacing: ".28em", color: "#FF4545", fontWeight: 600 }}>PUBLISHING FAQS</span>
                        </div>
    ...
    return (
        <>
            ...
            <section ref={ref} style={{ background: "linear-gradient(180deg, #1B465F 0%, #14384C 50%, #0E2432 100%)", width: "100%", overflow: "hidden", padding: "100px 0 110px", position: "relative" }}>
                <div style={{ position: "absolute", top: "15%", right: "-6%", width: "460px", height: "460px", borderRadius: "50%", background: "radial-gradient(circle, rgba(255,69,69,0.07) 0%, transparent 65%)", animation: "orbPulse 6s ease-in-out infinite", pointerEvents: "none" }} />
                <div style={{ position: "absolute", bottom: "10%", left: "-5%", width: "400px", height: "400px", borderRadius: "50%", background: "radial-gradient(circle, rgba(27, 70, 95, 0.55) 0%, transparent 70%)", pointerEvents: "none" }} />
                ...
                <div style={{ position: "absolute", top: "8%", left: "4%", width: "160px", height: "160px", border: "1px dashed rgba(255,69,69,0.1)", borderRadius: "50%", animation: "rotateSlow 22s linear infinite", pointerEvents: "none" }} />

                <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 40px" }}>
                    <div style={{ marginBottom: "64px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "20px", opacity: visible ? 1 : 0, animation: visible ? "fadeUpContact 0.6s ease forwards" : "none" }}>
                            <div style={{ height: "2px", background: "#FF4545", width: visible ? "48px" : "0", transition: "width 0.8s ease 0.2s" }} />
                            <span style={{ fontFamily: "'Montserrat', sans-serif", fontSize: "0.85rem", letterSpacing: "0.25em", color: "#FF4545" }}>PUBLISH YOUR BOOK</span>
                        </div>
                        <h2 style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 800, fontSize: "clamp(2.5rem, 4.5vw, 4.5rem)", letterSpacing: "-0.02em", lineHeight: 0.9, color: "white", margin: 0 }}>
                            {visible && (<>
                                <SplitText text="Get In" delay={35} duration={1.1} ease="power3.out" splitType="chars" from={{ opacity: 0, y: 45 }} to={{ opacity: 1, y: 0 }} threshold={0.1} rootMargin="-50px" textAlign="left" />{" "}
                                <SplitText text="Touch" className="text-[#FF4545]" delay={42} duration={1.2} ease="power3.out" splitType="chars" from={{ opacity: 0, y: 45 }} to={{ opacity: 1, y: 0 }} threshold={0.1} rootMargin="-50px" textAlign="left" /><br />
                                <SplitText text="And Take the Next Step" delay={42} duration={1.2} ease="power3.out" splitType="chars" from={{ opacity: 0, y: 45 }} to={{ opacity: 1, y: 0 }} threshold={0.1} rootMargin="-50px" textAlign="left" />
                            </>)}
                        </h2>
                    </div>

                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1.35fr", gap: "56px", alignItems: "start" }}>
                        <div style={{ opacity: visible ? 1 : 0, animation: visible ? "fadeLeft 0.9s ease 0.3s forwards" : "none" }}>
                            <div style={{ display: "flex", flexDirection: "column", gap: "14px", marginBottom: "48px" }}>
                                {INFO_ITEMS.map((item, i) => (
                                    <div key={i} className="info-card" style={{ display: "flex", alignItems: "center", gap: "16px", padding: "18px 22px", borderRadius: "14px", background: "rgba(255,255,255,0.028)", border: "1px solid rgba(255,255,255,0.07)", backdropFilter: "blur(6px)" }}>
                                        <div style={{ width: "44px", height: "44px", borderRadius: "12px", flexShrink: 0, background: "rgba(255,69,69,0.1)", border: "1px solid rgba(255,69,69,0.2)", display: "flex", alignItems: "center", justifyContent: "center", color: "#FF4545" }}>{item.icon}</div>
                                        <div>
                                            <p style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "0.75rem", letterSpacing: "0.15em", color: "rgba(255,255,255,0.35)", margin: "0 0 2px" }}>{item.label}</p>
                                            <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.92rem", color: "rgba(255,255,255,0.75)", margin: 0, fontWeight: 400 }}>{item.value}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div style={{ opacity: visible ? 1 : 0, animation: visible ? "fadeRight 0.9s ease 0.4s forwards" : "none" }}>
                            <div style={{ borderRadius: "24px", padding: "2px", background: "linear-gradient(135deg, rgba(255,69,69,0.35), rgba(26,36,95,0.6), rgba(255,69,69,0.2))" }}>
                                <div style={{ borderRadius: "22px", background: "linear-gradient(180deg, #1B465F 0%, #14384C 50%, #0E2432 100%)", padding: "44px 40px", position: "relative", overflow: "hidden" }}>
                                    ...
                                    {submitted ? (
                                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: "360px", gap: "20px", animation: "successPop 0.5s cubic-bezier(0.22,1,0.36,1) forwards" }}>
                                            <div style={{ width: "80px", height: "80px", borderRadius: "50%", background: "rgba(255,69,69,0.12)", border: "2px solid rgba(255,69,69,0.4)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                                <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#FF4545" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" style={{ strokeDasharray: 40, strokeDashoffset: 0, animation: "checkDraw 0.5s ease 0.3s both" }} /></svg>
                                            </div>
                                            <div style={{ textAlign: "center" }}>
                                                <p style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: "2rem", color: "white", margin: "0 0 8px", letterSpacing: "0.03em" }}>Message Sent!</p>
                                                <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "0.9rem", color: "#ffffff", margin: 0, fontWeight: 300 }}>We'll get back to you within 24 hours.</p>
                                            </div>
                                            <button onClick={() => { setSubmitted(false); setForm({ name: "", phone: "", email: "", message: "" }); }} style={{ fontFamily: "'Montserrat', sans-serif", letterSpacing: "0.1em", fontSize: "0.9rem", padding: "10px 28px", borderRadius: "999px", background: "transparent", color: "#ffffff", border: "1px solid rgba(255,255,255,0.15)", cursor: "pointer", marginTop: "8px" }}>SEND ANOTHER</button>
                                        </div>
                                    ) : (
                                        <div style={{ display: "flex", flexDirection: "column", gap: "20px", position: "relative", zIndex: 2 }}>
                                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
                                                <div className="field-wrap"><input className="field-input" type="text" name="name" placeholder=" " value={form.name} onChange={handleChange} autoComplete="off" /><span className="field-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" /><circle cx="12" cy="7" r="4" /></svg></span><label className="field-label">Full Name</label></div>
                                                <div className="field-wrap"><input className="field-input" type="tel" name="phone" placeholder=" " value={form.phone} onChange={handleChange} autoComplete="off" /><span className="field-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.8a19.79 19.79 0 01-3.07-8.7A2 2 0 012 .99h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 8.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" /></svg></span><label className="field-label">Phone Number</label></div>
                                            </div>
                                            <div className="field-wrap"><input className="field-input" type="email" name="email" placeholder=" " value={form.email} onChange={handleChange} autoComplete="off" /><span className="field-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" /><polyline points="22,6 12,13 2,6" /></svg></span><label className="field-label">Email Address</label></div>
                                            <div className="field-wrap"><textarea className="field-input" name="message" placeholder=" " value={form.message} onChange={handleChange} /><span className="field-icon field-icon-textarea"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" /></svg></span><label className="field-label">Your Message</label></div>
                                            <button className="submit-btn" onClick={handleSubmit} disabled={loading}>
                                                {loading ? (<><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" style={{ animation: "rotateSlow 0.8s linear infinite" }}><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83" /></svg>SENDING...</>) : (<><span style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 600, fontSize: '0.8rem' }}>Send Now</span><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 2L11 13M22 2L15 22l-4-9-9-4 20-7z" /></svg></>)}
                                            </button>
                                        </div>
                                    )}
                                </div>
